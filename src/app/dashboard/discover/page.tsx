...
function JobCard({ jobId }: { jobId: string }) {
 ...
 <p className="text-xs text-surface-300 mt-3 line-clamp-2 leading-relaxed">{(job.description || "No description")}</p>
...
export default function DiscoverPage() {
 ...
 <div className="text-[10px] text-surface-300 uppercase tracking-wider">{stat.label}</div>
 </div>
 </div>
 ))}
 </div>
 ...
 className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-surface-100 border border-surface-200 text-sm text-surface-400 placeholder:text-surface-300 focus:outline-none focus:border-brand-500/40 focus:ring-1 focus:ring-brand-500/20 transition-all"
 />
 </div>
 <button
 ...
 showFilters ? "bg-brand-500/10 text-brand-400" : "bg-surface-100 text-surface-300 hover:text-surface-400"
 )}
 >
 ...
 <div>
 <label className="text-[10px] text-surface-300 uppercase tracking-wider font-medium mb-1 block">Industry</label>
 <select
 value={store.filterIndustry}
 onChange={(e) => store.setFilterIndustry(e.target.value)}
 className="w-full px-3 py-2 rounded-lg bg-surface-200 text-sm text-surface-400 border border-white/[0.04] focus:outline-none"
 >
 ...
 <div>
 <label className="text-[10px] text-surface-300 uppercase tracking-wider font-medium mb-1 block">Location</label>
 <select
 ...
 <div>
 <label className="text-[10px] text-surface-300 uppercase tracking-wider font-medium mb-1 block">Level</label>
 <select
 ...
 <div>
 <label className="text-[10px] text-surface-300 uppercase tracking-wider font-medium mb-1 block">Min Match Score</label>
 <div className="flex items-center gap-2">
 <input
 type="range"
 min={0}
 ...
 />
 <span className="text-xs text-surface-300 font-mono w-8 text-right">{store.filterMinScore}%</span>
 </div>
 </div>
 </div>
 )}
 ...
 <h3 className="text-sm font-medium text-surface-300 mb-1">No matches found</h3>
 <p className="text-xs text-surface-400">Try adjusting your filters or run a new scan.</p>
 </div>
 ) : (
 ...
 <h3 className="text-sm font-medium text-surface-300 mb-1">No saved leads yet</h3>
 <p className="text-xs text-surface-400">Bookmark interesting jobs to save them here.</p>
 </div>
 ) : (
 ...
 <p className="text-xs text-surface-300">{topCompanies.length} companies tracked</p>
 <button
 ...
 <span className="text-[10px] px-1.5 py-0.5 rounded-md bg-emerald-500/10 text-emerald-300 font-medium">Active</span>
 </div>
 <button
 onClick={() => {
 setPrefsForm({
 ...
 className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-surface-200 text-surface-300 hover:text-brand-300 text-xs font-medium transition-all"
 >
 ...
 <div>
 <span className="text-surface-300">Target Roles:</span>
 <div className="text-surface-400 mt-0.5">{store.profile.target_roles.join(", ")}</div>
 </div>
 <div>
 <span className="text-surface-300">Industries:</span>
 <div className="text-surface-400 mt-0.5">{store.profile.industries.join(", ")}</div>
 </div>
 <div>
 <span className="text-surface-300">Locations:</span>
 <div className="text-surface-400 mt-0.5">{store.profile.locations.join(", ")}</div>
 </div>
 <div>
 <span className="text-surface-300">Min Match:</span>
 <div className="text-surface-400 mt-0.5">{store.profile.min_match_score}%</div>
 </div>
 <div>
 <span className="text-surface-300">Keywords:</span>
 <div className="text-surface-400 mt-0.5">{(store.profile?.keywords || []).slice(0, 3).join(", ") || "No keywords yet"}</div>
 </div>
 <div>
 <span className="text-surface-300">Auto-Scan:</span>
 ...
 <button onClick={() => setShowPrefsEditor(false)} className="p-1.5 rounded-lg text-surface-300 hover:text-surface-400 hover:bg-surface-0/[0.04] transition-all">
 <X className="w-4 h-4" />
 </button>
 </div>
 ...
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Target Roles</label>
 ...
 <p className="text-[10px] text-surface-400 mt-1">Comma-separated</p>
 </div>

 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Industries</label>
 ...
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Locations</label>
 ...
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Keywords</label>
 ...
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Experience Level</label>
 ...
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Min Match Score</label>
 <div className="flex items-center gap-2">
 ...
 <p className="text-[10px] text-surface-300">Automatically scan for new jobs</p>
 </div>
 </div>
 <div className="flex items-center gap-2">
 {prefsForm.auto_scan_enabled && (
 <select value={prefsForm.auto_scan_interval} onChange={(e) => setPrefsForm({ ...prefsForm, auto_scan_interval: e.target.value as "daily" | "weekly" | "biweekly" })} className="px-2 py-1 rounded-lg bg-surface-200 text-xs text-surface-400 border border-white/[0.04] focus:outline-none">
 ...
 <button onClick={() => setShowPrefsEditor(false)} className="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium text-surface-300 bg-surface-200 hover:bg-surface-300 transition-all">Cancel</button>
 ...
 <button onClick={() => setShowAddCompany(false)} className="p-1.5 rounded-lg text-surface-300 hover:text-surface-400 hover:bg-surface-0/[0.04] transition-all">
 <X className="w-4 h-4" />
 </button>
 </div>
 ...
 <input type="text" value={companyForm.logo_emoji} onChange={(e) => setCompanyForm({ ...companyForm, logo_emoji: e.target.value })} className="w-14 px-2 py-2.5 rounded-xl bg-surface-100 border border-surface-200 text-lg text-center focus:outline-none focus:border-brand-500/40 transition-all" />
 </div>
 <div className="flex-1">
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Company Name *</label>
 ...
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Industry</label>
 ...
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">HQ Location</label>
 ...
 <div className="grid grid-cols-2 gap-4">
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Employee Count</label>
 ...
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Tier</label>
 ...
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Career Page URL</label>
 ...
 <div>
 <label className="block text-xs font-medium text-surface-300 mb-1.5">Tags</label>
 ...
 <textarea value={companyForm.notes} onChange={(e) => setCompanyForm({ ...companyForm, notes: e.target.value })} rows={2} placeholder="Why this company interests you..." className="w-full px-3 py-2.5 rounded-xl bg-surface-100 border border-surface-200 text-sm text-surface-400 placeholder:text-surface-300 focus:outline-none focus:border-brand-500/40 transition-all resize-none" />
 </div>
 </div>

 <div className="flex gap-3 mt-6">
 <button onClick={() => setShowAddCompany(false)} className="flex-1 px-4 py-2.5 rounded-xl text-sm font-medium text-surface-300 bg-surface-200 hover:bg-surface-300 transition-all">Cancel</button>
 <button onClick={handleAddCompany} disabled={!companyForm.name.trim()} className={cn("flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all", companyForm.name.trim() ? "gradient-brand text-white hover:opacity-90" : "bg-surface-300 text-surface-300 cursor-not-allowed")}>
