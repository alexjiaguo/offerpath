    ...
    switch (storeName) {
      case "profile": {
        ...
        if (updateError || !updated || updated.length === 0) {
          // If no profile row exists yet, insert initial defaults
      ...
      case "resume": {
        const payload = Array.isArray(data)
          ? { resumes: data as Record<string, unknown>[] }
          : (data as { resumes?: Record<string, unknown>[]; atsEvaluations?: Record<string, unknown> });
        const resumes = payload.resumes ?? (Array.isArray(data) ? data : []);
        const atsEvaluations = (payload as { atsEvaluations?: Record<string, unknown> })?.atsEvaluations ?? {};
        ...
        if (dbResumes) {
          ...
          const storeIds = resumes.map((r) => r.id).filter((id) => isUUID(id));
        ...
        for (const resume of resumes) {
          ...
          const row: Record<string, unknown> = {
            ...
            ats_evaluations: resume.ats_evaluations ?? atsEvaluations ?? {},
          };
          await supabase
            .from("resumes")
      ...
      case "pipeline": {
        ...
        for (const company of companies) {
          if (!isUUID(company.id) || !company.name) continue;
          ...
          await supabase.from("companies").upsert(compRow, { onConflict: "id" });
        }

        // ── Active Deletion Sync for Jobs ──
        ...
        if (dbJobs) {
          ...
          const storeIds = jobs.map((j) => j.id).filter((id) => isUUID(id));
          const toDelete = dbIds.filter((id) => !storeIds.includes(id));
          if (toDelete.length > 0) {
        ...
        for (const job of jobs) {
          ...
          const row: Record<string, unknown> = {
            id: job.id,
            user_id: user.id,
            company_id: isUUID(job.company_id) ? job.company_id : null,
            title: job.title,
            ...
            resume_id: isUUID(job.resume_id) ? job.resume_id : null,
      ...
      case "discovery": {
        ...
        // Upsert discovered companies without deleting existing pipeline companies
        for (const company of companies) {
          if (!isUUID(company.id) || !company.name) continue;
          const row: Record<string, unknown> = {
            id: company.id,
      ...
      case "interview": {
        ...
        for (const prep of preps) {
          if (!isUUID(prep.id) || !isUUID(prep.job_id)) continue;
          try {
        ...
        // Mock sessions: active deletion + upsert with question_pool
        ...
        for (const session of mockSessions) {
          if (!isUUID(session.id)) continue;
          const validJobId = isUUID(session.job_id) ? session.job_id : null;
          try {
            await supabase.from("mock_sessions").upsert({
              id: session.id, user_id: user.id, job_id: validJobId,
              ...
              question_pool: session.questionPool ?? session.question_pool ?? [],
            }, { onConflict: "id" });
          } catch (err) {
            logger.warn(`[supabase-sync] mock session upsert failed:`, err);
          }
        }
        break;
      }
