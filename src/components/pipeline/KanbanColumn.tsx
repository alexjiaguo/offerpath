...
/* ═══════════════════════════════════════════════════
   KanbanColumn — Professional Minimalist Job Pipeline
   ═══════════════════════════════════════════════════ */
...
export default function KanbanColumn({ config, jobs, onAddClick }: KanbanColumnProps) {
  ...
  const columnTitle = (t.pipeline.columns as Record<string, string>)[config.id] || config.title;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <span className="text-[10px] font-bold text-surface-300 uppercase tracking-widest leading-none mt-1.5 block">
              {jobs.length} {isZh ? "个职位" : (jobs.length === 1 ? "Job" : "Jobs")}
            </span>
          </div>
        </div>
        ...
        >
          {jobs.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full min-h-[240px] text-center px-4">
              ...
              <p className="text-[11px] font-mono font-bold text-surface-300 uppercase tracking-widest mb-1">
              ...
              {config.id === "new" && onAddClick ? (
                ...
                <p className="text-[10px] text-surface-300 font-medium italic font-mono">
                  {t.pipeline.dragDropHint}
                </p>
              )}
            </div>
          ) : (
