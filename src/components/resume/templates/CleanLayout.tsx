...
const CleanLayout: React.FC<TemplateProps> = ({ data, theme, sectionOrder, sectionVisibility }) => {
 ...
 <div key={idx} style={{ marginBottom: `${theme.itemSpacing || 6}px`, fontSize: '10px' }}>
 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
 <span style={{ fontSize: '10px', fontWeight: 700, color: theme.primaryColor }}>{item.title}</span>
 ...
 <li key={i} style={{ fontSize: '10px', color: '#2e2e4a', marginBottom: '4px', lineHeight: theme.lineHeight || 1.3 }} dangerouslySetInnerHTML={{ __html: sanitizeHtml(b) }} />
 ))}
 </ul>
 </div>
 ))}
 ...
 <span><strong style={{ fontSize: '12px', fontWeight: 700, color: theme.primaryColor }}>{item.institution}{item.location ? `, ${item.location}` : ''}</strong> — <span style={{ fontSize: '10px', color: '#555', fontStyle: 'italic' }}>{item.degree}{item.field ? `, ${item.field}` : ''}</span></span>
 ...
 {showLang && <section><h3 style={{ ...sectionTitle, fontSize: '10px' }}>Languages</h3><div style={{ fontSize: '10px', color: '#3a3a5c' }}>{data.languages!.join(' · ')}</div></section>}
 {showCert && <section><h3 style={{ ...sectionTitle, fontSize: '10px' }}>Certifications</h3>{data.certifications!.map((c, i) => <div key={i} style={{ marginBottom: '3px', fontSize: '10px', color: '#3a3a5c' }}>{c}</div>)}</section>}
 </React.Fragment>
 );
 };
 ...
 <h1 style={{ fontSize: `${theme.headerFontSize || 28}px`, fontWeight: 700, color: theme.primaryColor, letterSpacing: '0.3px', marginBottom: '2px', lineHeight: 1.2 }}>{data.personal?.name}</h1>
 <div style={{ fontSize: '10px', color: '#5a5a7a', fontWeight: 500, marginBottom: '6px' }}>{data.personal?.title}</div>
