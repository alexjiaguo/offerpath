...
const BoldEngineer: React.FC<TemplateProps> = ({ data, theme, sectionOrder, sectionVisibility }) => {
 ...
 fontSize: `${theme.sectionTitleSize || 12}px`, fontWeight: 800,
 ...
 <div style={{ fontSize: `${theme.baseFontSize || 10}px`, lineHeight: theme.lineHeight || 1.3, marginBottom: '5px' }} dangerouslySetInnerHTML={{ __html: sanitizeHtml(data.summary) }} />
 </section>
 ) : null,
 ...
 <div key={idx} style={{ marginBottom: `${theme.itemSpacing || 7}px`, fontSize: `${theme.baseFontSize || 10}px`, lineHeight: theme.lineHeight || 1.3 }}>
 ...
 <li key={i} style={{ marginBottom: '4px', lineHeight: theme.lineHeight || 1.3 }} dangerouslySetInnerHTML={{ __html: sanitizeHtml(b) }} />
 ))}
 </ul>
 </div>
 ))}
 ...
 <span><strong style={{ fontWeight: 700, color: theme.primaryColor, fontSize: `${theme.companyFontSize || 11}px` }}>{item.institution}{item.location ? `, ${item.location}` : ""}</strong>, <em>{item.degree}{item.field ? `, ${item.field}` : ""}</em></span>
 ...
 {skills.map((s) => (
 <span key={s.id} style={{
 padding: '2px 8px', borderRadius: '3px', fontSize: `${theme.baseFontSize || 10}px`,
 ...
 {techSkills.map((cat) => (
 <div key={cat.id} style={{ marginBottom: '3px', fontSize: `${theme.baseFontSize || 10}px` }}>
 ...
 {showLang && <section>{secHeader('Languages')}<div style={{ fontSize: `${theme.baseFontSize || 10}px` }}>{data.languages!.join(' · ')}</div></section>}
 {showCert && <section>{secHeader('Certifications')}{data.certifications!.map((c, i) => <div key={i} style={{ marginBottom: '3px', fontSize: `${theme.baseFontSize || 10}px` }}>{c}</div>)}</section>}
 </div>
 );
 };
