...
const Academic: React.FC<TemplateProps> = ({ data, theme, sectionOrder, sectionVisibility }) => {
 ...
 summary: () => data.summary ? (<section key="summary" style={{ marginBottom: `${theme.sectionSpacing ?? 12}px` }}><EditableText field="summary" value={data.summary} html style={{ fontSize: `${theme.baseFontSize ?? 10}px`, color: '#3a3a3a', lineHeight: 1.5 }} /></section>) : null,
 ...
 <div style={{ marginBottom: `${theme.itemSpacing ?? 8}px`, position: 'relative' }}>
 ...
 return (<div key="lang-cert" style={{ display: 'grid', gridTemplateColumns: showLang && showCert ? '1fr 1fr' : '1fr', gap: '20px', marginBottom: `${theme.sectionSpacing ?? 12}px` }}>{showLang && <section><div style={sectionTitle}>Languages</div><div style={{ fontSize: '10px' }}>{(data.languages || []).map((l, li) => <React.Fragment key={li}>{li > 0 && ' · '}<EditableText field={`languages[${li}]`} value={l} /></React.Fragment>)}</div></section>}{showCert && <section><div style={sectionTitle}>Certifications</div>{(data.certifications || []).map((c, ci) => <div key={ci} style={{ marginBottom: `${Math.max(0, (theme.itemSpacing ?? 8) - 4)}px`, fontSize: '10px' }}><EditableText field={`certifications[${ci}]`} value={c} /></div>)}</section>}</div>);
 };
 ...
 <h1 style={{ fontSize: `${theme.headerFontSize ?? 28}px`, fontWeight: 700, color: theme.primaryColor, letterSpacing: '1.5px', marginBottom: '2px', lineHeight: 1.2 }}><EditableText field="personal.name" value={data.personal?.name} style={{ fontSize: `${theme.headerFontSize ?? 28}px`, fontWeight: 700, color: theme.primaryColor, letterSpacing: '1.5px', lineHeight: 1.2 }} /></h1>
 <div style={{ fontSize: '0.95em', color: '#555', marginTop: '4px' }}>
 {contactItems.map((item, i) => <React.Fragment key={i}>{i > 0 && ' · '}<EditableText field={item.field} value={item.value} style={{ fontSize: '0.95em', color: '#555' }} /></React.Fragment>)}
 </div>
 </header>
