...
const Academic: React.FC<TemplateProps> = ({ data, theme, sectionOrder, sectionVisibility }) => {
 ...
 {showLang && <section><div style={sectionTitle}>Languages</div><div style={{ fontSize: '10px' }}>{data.languages!.join(' · ')}</div></section>}
 {showCert && <section><div style={sectionTitle}>Certifications</div>{data.certifications!.map((c, i) => <div key={i} style={{ marginBottom: '3px', fontSize: '10px' }}>{c}</div>)}</section>}
 </div>
 );
 };
 ...
 <h1 style={{ fontSize: `${theme.headerFontSize || 28}px`, fontWeight: 700, color: theme.primaryColor, letterSpacing: '1.5px', marginBottom: '2px', lineHeight: 1.2 }}>{data.personal?.name}</h1>
