...
const ATSExecutive: React.FC<TemplateProps> = ({ data, theme, sectionOrder, sectionVisibility }) => {
 ...
 <EditableText field="summary" value={data.summary} html style={{ fontSize: '10px', color: '#3a3a3a', lineHeight: 1.5 }} />
 </section>
 ) : null,
    ...
    <section key="experience" style={{ marginBottom: `${theme.sectionSpacing ?? 12}px` }}>
      ...
      {data.experience!.map((item, idx) => (
        <DropZone key={idx} section="experience" index={idx}>
          <div style={{ marginBottom: `${theme.itemSpacing ?? 8}px`, lineHeight: theme.lineHeight ?? 1.3, position: 'relative' }}>
            ...
            <ul style={{ listStyleType: 'disc', paddingLeft: '16px', margin: '2px 0 0 0' }}>
              {item.bullets.map((b, i) => (
                <li key={i} style={{ fontSize: '10px', color: theme.textColor, marginBottom: bulletGap, lineHeight: theme.lineHeight ?? 1.3 }}>
                  <EditableText field={`experience[${idx}].bullets[${i}]`} value={b} html style={{ fontSize: '10px', color: theme.textColor, lineHeight: theme.lineHeight ?? 1.3 }} />
 ...
 {skills.map((s, si) => (
 <EditableSkillChip key={s.id} field={`skills[${si}].name`} value={s.name} section="skills" index={si} style={{ padding: '2px 8px', borderRadius: '3px', fontSize: '10px', backgroundColor: s.isHighlighted ? theme.primaryColor : '#f0f3f6', color: s.isHighlighted ? '#fff' : '#2c3e50', fontWeight: s.isHighlighted ? 600 : 400 }} />
 ))}
 ...
 <strong style={{ color: theme.primaryColor }}><EditableText field={`technicalSkills[${ti}].category`} value={cat.category} style={{ color: theme.primaryColor }} />:</strong> <EditableText field={`technicalSkills[${ti}].skills`} value={cat.skills} />
 ...
 {showLang && (<section><h3 style={{ ...sectionTitle, fontSize: '10px' }}>Languages</h3><div style={{ fontSize: '10px' }}>{(data.languages || []).map((l, li) => <React.Fragment key={li}>{li > 0 && ' · '}<EditableText field={`languages[${li}]`} value={l} /></React.Fragment>)}</div></section>)}
 {showCert && (<section><h3 style={{ ...sectionTitle, fontSize: '10px' }}>Certifications</h3>{(data.certifications || []).map((c, ci) => <div key={ci} style={{ marginBottom: `${Math.max(0, (theme.itemSpacing ?? 8) - 4)}px`, fontSize: '10px' }}><EditableText field={`certifications[${ci}]`} value={c} /></div>)}</section>)}
 </div>
 );
 };
 ...
 <h1 style={{ fontSize: `${theme.headerFontSize ?? 28}px`, fontWeight: 700, color: theme.primaryColor, letterSpacing: '0.3px', marginBottom: '1px', lineHeight: 1.2 }}><EditableText field="personal.name" value={data.personal?.name} style={{ fontSize: `${theme.headerFontSize ?? 28}px`, fontWeight: 700, color: theme.primaryColor, letterSpacing: '0.3px', lineHeight: 1.2 }} /></h1>
 <div style={{ fontSize: '12px', color: theme.accentColor, fontWeight: 600, marginBottom: '4px' }}><EditableText field="personal.title" value={data.personal?.title} style={{ fontSize: '12px', color: theme.accentColor, fontWeight: 600 }} /></div>
 <div style={{ fontSize: '10px', color: '#555' }}>
 {contactItems.map((item, i) => <React.Fragment key={i}>{i > 0 && <span style={{ color: theme.accentColor }}> · </span>}<EditableText field={item.field} value={item.value} style={{ fontSize: '10px', color: '#555' }} /></React.Fragment>)}
 </div>
 </header>
