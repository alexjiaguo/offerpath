...
const CleanProfessional: React.FC<TemplateProps> = ({ data, theme, sectionOrder, sectionVisibility }) => {
 ...
 <div key={idx} style={{ marginBottom: '7px', fontSize: `${theme.baseFontSize || 10}px`, lineHeight: theme.lineHeight || 1.3 }}>
 ...
 {item.bullets.map((b, i) => <li key={i} style={{ marginBottom: '4px', lineHeight: theme.lineHeight || 1.3 }} dangerouslySetInnerHTML={{ __html: sanitizeHtml(b) }} />)}
 </ul>
 </div>
 ))}
 </section>
 ...
 <span><strong style={{ color: theme.primaryColor, fontSize: "10px", fontWeight: 700 }}>{item.institution}{item.location ? `, ${item.location}` : ""}</strong>, <em>{item.degree}{item.field ? `, ${item.field}` : ""}</em></span>
 ...
 {showLang && <section><h2 style={sectionTitle}>Languages</h2><div style={{ fontSize: '10px' }}>{data.languages!.join(' · ')}</div></section>}
 {showCert && <section><h2 style={sectionTitle}>Certifications</h2>{data.certifications!.map((c, i) => <div key={i} style={{ marginBottom: '3px', fontSize: '10px' }}>{c}</div>)}</section>}
 </div>
 );
 };
